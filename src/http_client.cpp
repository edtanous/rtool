...
namespace http
{
    ...
    void ConnectionInfo::doSslHandshake()
    {
        ...
        sslConn->async_handshake(
            ...
            std::bind_front(
                &ConnectionInfo::afterSslHandshake, this, shared_from_this()));
    }
    ...
    void ConnectionInfo::onIdleEvent(
        const std::weak_ptr<ConnectionInfo>& /*self*/,
        const boost::system::error_code& ec)
    {
        if (ec && ec != boost::asio::error::operation_aborted) { doClose(); }
    }

    void ConnectionInfo::afterWrite(
        const std::shared_ptr<ConnectionInfo>& /*self*/,
        ...
        const boost::beast::http::verb verb)
    {
        ...
        sendDataWithCallback(
            std::move(data), id, destIP, destPort, destUri, useSSL, httpHeader,
            verb, cb);
    }
